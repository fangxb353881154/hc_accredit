...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/sys/authorKey/list2">授权码列表</a></li>
		<shiro:hasPermission name="sys:authorKey:edit"><li><a href="${ctx}/sys/authorKey/form">授权码添加</a></li></shiro:hasPermission>
	</ul>
	<form:form id="searchForm" modelAttribute="authorKey" action="${ctx}/sys/authorKey/list2" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="authorKey">
			<tr>
				...
				<td>
					<fmt:formatDate value="${authorKey.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<fmt:formatDate value="${authorKey.laseDate}" pattern="yyyy-MM-dd HH:mm:ss"/>
				</td>
				<td>
					<a href="${ctx}/sys/authorKey/delete?id=${authorKey.id}" onclick="return confirmx('确认要删除该授权码？', this.href)">删除</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
